...
def get_api_url(endpoint):
    base = settings.RESTAPIBASEURL.rstrip('/') + '/'
    return urljoin(base, endpoint.strip('/') + '/')
...
def eliminar_pago(idPago):
    ...
    :return True if succesful,
     False otherwise
     """
    api_url = get_api_url(f'pago/{idPago}')
    try:
...
def get_pagos_from_db(idComercio):
    ...
    :return list of pagos found
     """
    api_url = get_api_url(f'comercio/{idComercio}')
    try:
